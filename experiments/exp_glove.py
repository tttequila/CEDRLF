...
class DDPG(object):
    ...
    def save_ckpt(self):
        ...
        :return: None
        """
        if not os.path.exists('model_glove'):
            os.makedirs('model_glove')

        tl.files.save_weights_to_hdf5('model_glove/ddpg_actor.hdf5', self.actor)
        tl.files.save_weights_to_hdf5('model_glove/ddpg_actor_target.hdf5', self.actor_target)
        tl.files.save_weights_to_hdf5('model_glove/ddpg_critic.hdf5', self.critic)
        tl.files.save_weights_to_hdf5('model_glove/ddpg_critic_target.hdf5', self.critic_target)
    ...
    def load_ckpt(self):
        ...
        :return: None
        """
        tl.files.load_hdf5_to_weights_in_order('model_glove/ddpg_actor.hdf5', self.actor)
        tl.files.load_hdf5_to_weights_in_order('model_glove/ddpg_actor_target.hdf5', self.actor_target)
        tl.files.load_hdf5_to_weights_in_order('model_glove/ddpg_critic.hdf5', self.critic)
        tl.files.load_hdf5_to_weights_in_order('model_glove/ddpg_critic_target.hdf5', self.critic_target)
...
if __name__ == '__main__':
  ...
  # p = env.reset()
    ...
    with open("correct rate_glove.txt", "w") as f:
        f.write(str(total))
        f.write('\n')
        f.write(str(corr))
